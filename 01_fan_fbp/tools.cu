...
void recImg(float *d_pws, cufftComplex *H, float *d_rec_img, Geo geo)
{
    ...
    cufftReal *h_truncated_proj = new cufftReal[geo.detector_num];

    // 创建纹理对象，将投影数据绑定到纹理对象
    cudaChannelFormatDesc channelDesc = cudaCreateChannelDesc<float>();
    cudaArray *cuArray_proj;
    cudaMallocArray(&cuArray_proj, &channelDesc, geo.detector_num,  1);
    cudaTextureObject_t texObj_proj = bindTexObj(cuArray_proj, h_truncated_proj, geo.detector_num, 1);
    ...
    for (int i = 0; i < geo.views; i++)
    {
        ...
        cufftExecR2C(plan_r2c, &d_pws[i * geo.padding_num], d_proj);
        cufftDestroy(plan_r2c);
        // 滤波
        filterProj<<<(geo.padding_num / 2 + 1 + 255) / 256, 256>>>(d_proj, H, geo);
        // 傅里叶逆变换
        cufftHandle plan_c2r;
        cufftPlan1d(&plan_c2r, geo.padding_num, CUFFT_C2R, 1);
        cufftExecC2R(plan_c2r, d_proj, d_real_proj);
        cufftDestroy(plan_c2r);
        cudaMemcpy(h_real_proj, d_real_proj, geo.padding_num * sizeof(cufftReal), cudaMemcpyDeviceToHost);
        // 归一化
        for (int j = 0; j < geo.padding_num; j++)
        {
            h_real_proj[j] /= geo.padding_num;
        }
        // 截取
        for (int j = 0; j < geo.detector_num; j++)
        {
            h_truncated_proj[j] = h_real_proj[j + geo.detector_num / 2];
        }
        // 投影数据绑定到纹理对象
        cudaMemcpyToArray(cuArray_proj, 0, 0, h_truncated_proj, geo.detector_num * sizeof(cufftReal), cudaMemcpyHostToDevice);
        // 重建
        backProject<<<grid_rec, block_rec>>>(d_rec_img, geo, i * geo.da, texObj_proj);
    }

    delete[] h_real_proj;
    delete[] h_truncated_proj;
    cudaFree(d_proj);
    cudaFree(d_real_proj);
}

void getSino(float *d_sino, float *d_interp_img, Geo geo, cudaTextureObject_t texObj)
{
    dim3 block(16, 16);
    dim3 grid((geo.detector_num + block.x - 1) / block.x, ((2 * geo.r / geo.step_size + 1) + block.y - 1) / block.y);
    dim3 block2(32);
    dim3 grid2((geo.detector_num + block2.x - 1) / block2.x);
    for (int view = 0; view < geo.views; view++)
    {
        getInterpImg<<<grid, block>>>(d_interp_img, view * geo.da, geo, texObj);
        sumInterpImg<<<grid2, block2>>>(d_sino, d_interp_img, view, geo);
    }
}
