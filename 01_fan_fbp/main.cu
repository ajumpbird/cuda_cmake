...
int main()
{
    ...
    // 插值图像
    int interp_rows = 2 * geo.r / geo.step_size +1;
    int interp_cols = geo.detector_num;
    int interp_size = interp_rows * interp_cols;
    float *d_interp_img;
    cudaMalloc(&d_interp_img, interp_size * sizeof(float));
    ...
    // 生成 sinogram
    getSino(d_sino, d_interp_img, geo, texObj);
